...
import { useServiceResourcePermissions, useProjectResourcePermissions, useUsers, useUsersByIds, useGrantResourcePermission, useRevokeResourcePermission } from '../../api/client';
...
export default function PermissionsTab({ serviceId, serviceOwnerId, projectId, projectCreatedAt, projectUpdatedAt, canManagePermissions, resourcePermissions: resourcePermissionsFromProps }: PermissionsTabProps) {
  const { user: currentUser } = useAuth();
  const { data: permissionsFallback, isLoading: permissionsLoading } = useServiceResourcePermissions(
    serviceId,
  ...
  // This is needed even when using props because props only contain service-level permissions
  ...
  // Filter to only service-level permissions (props might include all permissions)
  const servicePermissions = React.useMemo(() => {
    const perms = resourcePermissionsFromProps || permissionsFallback || [];
    // Filter to only service-level permissions
    return perms.filter(perm => perm.scope === 'service');
  ...
  // Combine service and project permissions
  // Project permissions are inherited, so we show them with 'project' scope
  // Service permissions are explicit, so we show them with 'service' scope
  // If a user has both, prefer the service-level permission (more specific)
  const allPermissions = React.useMemo(() => {
    // servicePermissions is already filtered to scope='service'
    const svcPerms = (servicePermissions || []).map(perm => ({ ...perm, _source: 'service' }));
    ...
    // Combine and deduplicate: if user has both project and service permissions, keep service one
    const permissionMap = new Map<string, typeof svcPerms[0]>();
    
    // First add project permissions
    projPerms.forEach(perm => {
      if (!serviceOwnerId || perm.userId !== serviceOwnerId) {
        permissionMap.set(perm.userId, perm);
      }
    });
    
    // Then add/override with service permissions (more specific)
    svcPerms.forEach(perm => {
      if (!serviceOwnerId || perm.userId !== serviceOwnerId) {
        permissionMap.set(perm.userId, perm);
      }
    });
  ...
  const filteredUsers = React.useMemo(() => {
    ...
    if (serviceOwnerId) {
      users = users.filter(user => user.id !== serviceOwnerId);
    }
    
    return users;
  }, [allUsers, serviceOwnerId]);
  ...
  const resourcePermissionsWithUsers = React.useMemo(() => {
    const mapped = allPermissions.map(perm => {
      ...
      const scope: 'project' | 'service' = perm._source === 'project' ? 'project' : 'service';
      return {
        ...perm,
  ...
  const ownerUser = serviceOwnerId 
    ? (users.find((u: { id: string }) => u.id === serviceOwnerId) || null)
    : null;
  const ownerPermission = serviceOwnerId ? {
  ...
  const confirmRevoke = async () => {
    ...
    try {
      await revokeMutation.mutateAsync({
        ...
        resourceId: serviceId,
        scope: revokePermissionScope || 'service', // Default to service scope
      });
              ...
              <tbody>
                {filteredPermissions.map((perm) => (
                  ...
                  >
                    ...
                    {/* For service-level permissions: show revoke button or self message */}
                    {!perm.isOwner && !perm.isAuthenticatedUsers && perm.permissionScope === 'service' && currentUser?.id === perm.userId && (
                      <td style={{ padding: 'var(--chakra-spacing-sm)' }}>
                    ...
                    {canManagePermissions && !perm.isOwner && !perm.isAuthenticatedUsers && currentUser?.id !== perm.userId && perm.permissionScope === 'service' && (
                      <td style={{ padding: 'var(--chakra-spacing-sm)' }}>
                        <Button
                          size="2xs"
                          variant="outline"
